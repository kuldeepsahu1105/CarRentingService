...
String fueltype=request.getParameter("fueltype");
        PreparedStatement ps = con.prepareStatement("SELECT count(id) as cnt from tblvehicles where tblvehicles.VehiclesBrand=? and tblvehicles.FuelType=?");
...
%>
<p><span><%= rs.getString("cnt") %> Listings</span></p>
</div>
</div>

<%  
        PreparedStatement ps1 = con.prepareStatement("SELECT tblvehicles.*,tblbrands.BrandName,tblbrands.id as bid  from tblvehicles join tblbrands on tblbrands.id=tblvehicles.VehiclesBrand where tblvehicles.VehiclesBrand=? and tblvehicles.FuelType=?");
        ps1.setString(1, brand);
        ps1.setString(2, fueltype);
...
%>
        ...
        <div class="product-listing-m gray-bg">
          ...
          <div class="product-listing-content">
            ...
            <p class="list-price">$<%= rs1.getString("PricePerDay") %> Per Day</p>
            <ul>
              <li><i class="fa fa-user" aria-hidden="true"></i><%= rs1.getString("SeatingCapacity") %> seats</li>
              <li><i class="fa fa-calendar" aria-hidden="true"></i><%= rs1.getString("ModelYear") %> model</li>
...
%>
              ...
              <li class="gray-bg">
                ...
                <div class="recent_post_title"> <a href="vehical-details.jsp?vhid=<%= rs3.getInt("id") %>"><%= rs3.getString("BrandName") %> , <%= rs3.getString("VehiclesTitle") %></a>
                  <p class="widget_price">$<%= rs3.getInt("PricePerDay") %> Per Day</p>
                </div>
              </li>
              <% } %>
